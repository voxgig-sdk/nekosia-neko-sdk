...
module NekosiaNekoUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = NekosiaNekoResponse.new({})
    ...
    if fetch_err
      ...
      response = NekosiaNekoResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = NekosiaNekoResponse.new(fetched)
    else
