# NekosiaNeko Ruby SDK

The Ruby SDK for the NekosiaNeko API. Provides an entity-oriented interface using idiomatic Ruby conventions.
...
```ruby
result, err = client.Booru(nil).load({ "id" => "example_id" }, nil)
raise err if err
puts result
```
...
```ruby
# Create
created, _ = client.Booru(nil).create({ "name" => "Example" }, nil)
...
result, err = client.NekosiaNeko(nil).load(
  { "id" => "test01" }, nil
)
