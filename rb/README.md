...
The Ruby SDK for the NekosiaNeko API — an entity-oriented client using idiomatic Ruby conventions.
...
raise a `NekosiaNekoError` (a `StandardError` subclass), so wrap
...
```ruby
# list returns an Array of Booru records (raises on error).
boorus = client.Booru.list
```
