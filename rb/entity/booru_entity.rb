...
class BooruEntity
  ...
  def make
    opts = @_entopts.dup
    BooruEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = NekosiaNekoHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = NekosiaNekoHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  #   as an empty match, so client.Booru.load works with no args).
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = NekosiaNekoHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
  ...
  #
  # @param reqmatch [BooruListMatch, Hash, nil] match filter (any subset of
  #   Booru fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
