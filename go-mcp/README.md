...
[MCP](https://modelcontextprotocol.io) server exposing the NekosiaNeko SDK as
two agent tools — `nekosia-neko_list` and `nekosia-neko_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/nekosia-neko-mcp)
make build

# 2. Provide credentials via the environment
export NEKOSIA_NEKO_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user nekosia-neko \
  -- /absolute/path/to/nekosia-neko-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./nekosia-neko-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// nekosia-neko_list: first page of records
{ "entity": "booru" }
{ "entity": "booru", "query": { } }

// nekosia-neko_load: one record by id
{ "entity": "booru", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/nekosia-neko-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user nekosia-neko \
     -- "$PWD"/dist/*/nekosia-neko-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `nekosia-neko_list` and `nekosia-neko_load` tools now appear
   in new sessions. Ask the agent to *"list booru using nekosia-neko"*
   and it calls `nekosia-neko_list` with `{"entity":"booru"}`.
...
```sh
export NEKOSIA_NEKO_APIKEY=sk_live_xxx            # API key
export NEKOSIA_NEKO_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `nekosia-neko_list` tool
...
### Call the `nekosia-neko_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `nekosia-neko_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `nekosia-neko_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `NEKOSIA_NEKO_APIKEY` | API key sent with every request. |
| `NEKOSIA_NEKO_BASE` | Optional override of the API base URL. |
...
```sh
./nekosia-neko-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"nekosia-neko_load","arguments":{"entity":"booru","query":{"id":1}}}}'
```
