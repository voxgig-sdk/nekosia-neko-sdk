...
local NekosiaNekoSDK = {}
NekosiaNekoSDK.__index = NekosiaNekoSDK
...
NekosiaNekoSDK._make_feature = _make_feature


function NekosiaNekoSDK.new(options)
  local self = setmetatable({}, NekosiaNekoSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function NekosiaNekoSDK:options_map()
...
function NekosiaNekoSDK:get_utility()
  return Utility.copy(self._utility)
end


function NekosiaNekoSDK:get_root_ctx()
  return self._rootctx
end


function NekosiaNekoSDK:prepare(fetchargs)
...
function NekosiaNekoSDK:direct(fetchargs)
...
-- by passing it.
function NekosiaNekoSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function NekosiaNekoSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "NekosiaNekoSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function NekosiaNekoSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = NekosiaNekoSDK.new(sdkopts)
  sdk.mode = "test"
