...
describe("BooruDirect", function()
  it("should direct-list-booru", function()
    local setup = booru_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-booru", setup.live and "live" or "unit")
  ...
  it("should direct-load-booru", function()
    local setup = booru_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-booru", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function booru_direct_setup(mockres)
