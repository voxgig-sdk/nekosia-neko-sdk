...
boru-driven command-line client **and** interactive REPL for the NekosiaNeko
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/nekosia-neko-cli)
make build

# 2. See usage (words, entities, env vars)
./nekosia-neko-cli --help

# 3. Provide credentials once, via the environment
export NEKOSIA_NEKO_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./nekosia-neko-cli list booru
./nekosia-neko-cli load 1 booru            # {id:1} shorthand
./nekosia-neko-cli load '{id:1}' booru       # explicit match map
./nekosia-neko-cli list image

# 5. Override the API base URL for a single call
NEKOSIA_NEKO_BASE=https://api.example.com ./nekosia-neko-cli list booru

# 6. No arguments -> interactive REPL
./nekosia-neko-cli
nekosia-neko> list booru
nekosia-neko> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/nekosia-neko-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/nekosia-neko-cli list booru
   ```
...
```sh
./nekosia-neko-cli load 1 booru          # scalar shorthand for {id:1}
./nekosia-neko-cli load '{id:1}' booru     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `nekosia-neko>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `booru`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/nekosia-neko-cli`. |
...
(`./nekosia-neko-cli <expr>`) and interactively (the REPL), and expressions compose the
