// Command NekosiaNeko-cli is a CLI + REPL for the NekosiaNeko SDK,
...
const prompt = "nekosia-neko"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "booru image"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      NEKOSIA_NEKO_APIKEY (api key), NEKOSIA_NEKO_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: NEKOSIA_NEKO_APIKEY carries the API key and
	// NEKOSIA_NEKO_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("NEKOSIA_NEKO_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("NEKOSIA_NEKO_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewNekosiaNekoSDK(opts)
