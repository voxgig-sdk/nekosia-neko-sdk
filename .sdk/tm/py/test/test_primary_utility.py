...
from projectname_sdk import NekosiaNekoSDK
from projectname_sdk.core.spec import NekosiaNekoSpec
from projectname_sdk.core.result import NekosiaNekoResult
from projectname_sdk.core.response import NekosiaNekoResponse
from projectname_sdk.core.operation import NekosiaNekoOperation
from projectname_sdk.core.error import NekosiaNekoError
from projectname_sdk.core import helpers
from projectname_sdk.feature.base_feature import NekosiaNekoBaseFeature
...
def _err_from_map(m):
    ...
    if msg == "":
        return None
    code = m.get("code", "")
    return NekosiaNekoError(code, msg)
...
class TestPrimaryUtility:
    ...
    def test_feature_init_basic(self):
        init_client = NekosiaNekoSDK.test(None, None)
        ...
        class TestInitFeature(NekosiaNekoBaseFeature):
            def __init__(self):
                super().__init__()
                self.name = "initfeat"
                self.active = True
    ...
    def test_feature_init_inactive(self):
        ...
        class TestInitFeatureInactive(NekosiaNekoBaseFeature):
            def __init__(self):
                super().__init__()
                self.name = "nofeat"
                self.active = False
    ...
    def test_make_result_no_result(self):
        ...
        utility = client._utility
        ctx = _make_test_full_ctx(client, utility)
        ctx.spec = NekosiaNekoSpec({"step": "start"})
        ctx.result = None
    ...
    def test_make_spec_basic(self):
        ...
        spec_client = NekosiaNekoSDK.test(None, setup_opts)
    ...
    def test_operator_basic(self):
        ...
        def subject(entry):
            ...
            if not isinstance(in_val, dict):
                in_val = {}
            op = NekosiaNekoOperation(in_val)
            return {
                "entity": op.entity,
                "name": op.name,
                "input": op.input,
    ...
    def test_prepare_auth_basic(self):
        ...
        auth_client = NekosiaNekoSDK.test(None, setup_opts)
