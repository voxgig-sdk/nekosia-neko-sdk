...
{
    private static function utility(): NekosiaNekoUtility
    {
        return new NekosiaNekoUtility();
    }
    ...
    private static function ctx(array $over = []): NekosiaNekoContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new NekosiaNekoContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new NekosiaNekoOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof NekosiaNekoError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new NekosiaNekoOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = NekosiaNekoMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new NekosiaNekoOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = NekosiaNekoMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new NekosiaNekoOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = NekosiaNekoMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new NekosiaNekoSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = NekosiaNekoMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new NekosiaNekoResponse([]),
            'result' => new NekosiaNekoResult([]),
        ]);
        [, $err] = NekosiaNekoMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new NekosiaNekoSpec([]),
            'response' => null,
            'result' => new NekosiaNekoResult([]),
        ]);
        [, $err] = NekosiaNekoMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new NekosiaNekoSpec(['step' => 's']),
            'response' => new NekosiaNekoResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new NekosiaNekoResult([]),
        ]);
        [, $err] = NekosiaNekoMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new NekosiaNekoSpec(['step' => 's']),
            'response' => new NekosiaNekoResponse(self::resp(200, ['v' => 1])),
            'result' => new NekosiaNekoResult([]),
        ]);
        [, $err] = NekosiaNekoMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new NekosiaNekoSpec(['step' => 's']),
            'response' => new NekosiaNekoResponse(self::resp(200, ['v' => 2])),
            'result' => new NekosiaNekoResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        NekosiaNekoMakeResponse::call($ctx);
    ...
    {
        $preset = new NekosiaNekoResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new NekosiaNekoSpec([]),
            'response' => new NekosiaNekoResponse([]),
            'result' => new NekosiaNekoResult([]),
        ...
        [$got, $err] = NekosiaNekoMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new NekosiaNekoResult([])]);
        [, $err] = NekosiaNekoMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new NekosiaNekoSpec([]), 'result' => null]);
        [, $err] = NekosiaNekoMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new NekosiaNekoSpec(['step' => 's']),
            'result' => new NekosiaNekoResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new NekosiaNekoOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = NekosiaNekoMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new NekosiaNekoSpec(['step' => 's']),
            'result' => new NekosiaNekoResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new NekosiaNekoOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = NekosiaNekoMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new NekosiaNekoResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new NekosiaNekoSpec([]),
            'result' => new NekosiaNekoResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = NekosiaNekoMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = NekosiaNekoMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new NekosiaNekoError('boom', 'boom');
        $utility->fetcher = function (NekosiaNekoContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new NekosiaNekoSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = NekosiaNekoMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (NekosiaNekoContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = NekosiaNekoMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(NekosiaNekoResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        NekosiaNekoMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (NekosiaNekoContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new NekosiaNekoSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = NekosiaNekoMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new NekosiaNekoResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new NekosiaNekoSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = NekosiaNekoMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = NekosiaNekoMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = NekosiaNekoMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new NekosiaNekoResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, NekosiaNekoDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new NekosiaNekoResult(['ok' => false])]);
        $this->expectException(NekosiaNekoError::class);
        NekosiaNekoDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new NekosiaNekoResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', NekosiaNekoMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        NekosiaNekoMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new NekosiaNekoBaseFeature();
        $b = new NekosiaNekoBaseFeature();
        NekosiaNekoFeatureAdd::call($ctx, $a);
        NekosiaNekoFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): NekosiaNekoBaseFeature
    {
        $f = new NekosiaNekoBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        NekosiaNekoFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): NekosiaNekoContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new NekosiaNekoSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = NekosiaNekoPrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        NekosiaNekoPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new NekosiaNekoResponse(['headers' => null]),
            'result' => new NekosiaNekoResult([]),
        ]);
        NekosiaNekoResultHeaders::call($ctx);
    ...
    public function test_result_body_skips_parsing_when_the_body_is_absent(): void
    {
        ...
        NekosiaNekoResultBody::call($ctx);
