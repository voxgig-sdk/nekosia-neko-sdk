...
class NekosiaNekoMakeError
{
    public static function call(?NekosiaNekoContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new NekosiaNekoContext([], null);
        }
        $op = $ctx->op ?? new NekosiaNekoOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new NekosiaNekoResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof NekosiaNekoError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new NekosiaNekoError('', $msg, $ctx);
        ...
        if ($err instanceof NekosiaNekoError) {
