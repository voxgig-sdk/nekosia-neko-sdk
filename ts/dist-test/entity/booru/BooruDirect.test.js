...
(0, node_test_1.describe)('BooruDirect', async () => {
    ...
    (0, node_test_1.test)('direct-exists', async () => {
        const sdk = new __1.NekosiaNekoSDK({
    ...
    (0, node_test_1.test)('direct-load-booru', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup({ id: 'direct01' });
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-load-booru', setup.live))
            return;
    ...
    (0, node_test_1.test)('direct-list-booru', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }]);
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-list-booru', setup.live))
            return;
        const { client, calls } = setup;
        const params = {};
        const query = {};
...
function liveScenariosActive() { return false && process.env.NEKOSIA_NEKO_TEST_LIVE === 'TRUE'; }
function directSetup(mockres) {
    ...
    if (live) {
        ...
        const client = new __1.NekosiaNekoSDK(Object.assign({}, (0, utility_1.liveClientOptions)(), { system: { fetch: transport.fetch },
        }));
        let idmap = env['NEKOSIA_NEKO_TEST_BOORU_ENTID'];
    ...
    const client = new __1.NekosiaNekoSDK({
        base: 'http://localhost:8080',
...
//# sourceMappingURL=BooruDirect.test.js.map