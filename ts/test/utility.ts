...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.NEKOSIANEKO_TEST_LIVE ||
    'TRUE' === process.env.NEKOSIANEKO_TEST_OVERRIDE
  ) {
  ...
  m.NEKOSIANEKO_TEST_EXPLAIN = process.env.NEKOSIANEKO_TEST_EXPLAIN || m.NEKOSIANEKO_TEST_EXPLAIN
