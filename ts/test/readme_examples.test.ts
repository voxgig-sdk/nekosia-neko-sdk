...
//     (\`new NekosiaNekoSDK(...)\` / \`NekosiaNekoSDK.test(...)\` become
//     \`NekosiaNekoSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"booru":{"test01":{"id":"test01"}},"image":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new NekosiaNekoSDK(...)\` and any
// \`NekosiaNekoSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+NekosiaNekoSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/NekosiaNekoSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
