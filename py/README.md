# NekosiaNeko Python SDK

The Python SDK for the NekosiaNeko API. Provides an entity-oriented interface following Pythonic conventions.
...
```python
result, err = client.Booru(None).list(None, None)
if err:
    raise Exception(err)
...
```python
result, err = client.Booru(None).load({"id": "example_id"}, None)
if err:
    raise Exception(err)
print(result)
```
...
```python
# Create
created, _ = client.Booru(None).create({"name": "Example"}, None)
...
result, err = client.NekosiaNeko(None).load(
    {"id": "test01"}, None
)
...
```python
client = NekosiaNekoSDK.test(testopts, sdkopts)
```
...
The main module (`nekosianeko_sdk`) exports the SDK class.
