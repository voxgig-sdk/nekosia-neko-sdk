...
class TestBooruEntity:
    ...
    def test_should_stream(self):
        ...
        base = NekosiaNekoSDK.test(seed, None)
        seen = list(base.Booru(None).stream("list", None, None))
        assert len(seen) == 3

        # Inbound: streaming active -> yields each item from the feature.
        from nekosianeko_sdk.config import shared_config
        cfg = shared_config()
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            sdk = NekosiaNekoSDK.test(
                seed, {"feature": {"streaming": {"active": True}}})
            got = []
            for item in sdk.Booru(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        setup = _booru_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["create", "list", "load"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "booru." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set NEKOSIA_NEKO_TEST_BOORU_ENTID JSON to run live")
        client = setup["client"]

        # CREATE
        booru_ref01_ent = client.Booru(None)
        booru_ref01_data = helpers.to_map(vs.getprop(
            vs.getpath(setup["data"], "new.booru"), "booru_ref01"))

        booru_ref01_data = helpers.to_map(runner.entity_data(booru_ref01_ent.create(booru_ref01_data, None)))
        ...
        booru_ref01_data_dt0_load_result = helpers.to_map(runner.entity_data(booru_ref01_data_dt0_loaded))
        assert booru_ref01_data_dt0_load_result is not None
        assert booru_ref01_data_dt0_load_result["id"] == booru_ref01_data["id"]



def _booru_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/booru/BooruTestData.json")
    ...
    idmap_resolved = helpers.to_map(
        env.get("NEKOSIA_NEKO_TEST_BOORU_ENTID"))
    if idmap_resolved is None:
        idmap_resolved = helpers.to_map(idmap)

    if env.get("NEKOSIA_NEKO_TEST_LIVE") == "TRUE":
        merged_opts = vs.merge([
            {
            },
            extra or {},
        ])
        client = NekosiaNekoSDK(helpers.to_map(merged_opts))

    _live = env.get("NEKOSIA_NEKO_TEST_LIVE") == "TRUE"
    return {
        "client": client,
        "data": entity_data,
        "idmap": idmap_resolved,
        "env": env,
        "explain": env.get("NEKOSIA_NEKO_TEST_EXPLAIN") == "TRUE",
        "live": _live,
