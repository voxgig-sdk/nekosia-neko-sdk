...
class NekosiaNekoTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            NekosiaNekoTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = NekosiaNekoTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = NekosiaNekoTestRunner.getenv("NEKOSIA_NEKO_TEST_LIVE")
        override = NekosiaNekoTestRunner.getenv("NEKOSIA_NEKO_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = NekosiaNekoTestRunner.getenv(key)
        ...
        explain = NekosiaNekoTestRunner.getenv("NEKOSIA_NEKO_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["NEKOSIA_NEKO_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if NekosiaNekoTestRunner._test_control is not None:
            return NekosiaNekoTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                NekosiaNekoTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            NekosiaNekoTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = NekosiaNekoTestRunner.load_test_control()
...
def load_env_local():
    NekosiaNekoTestRunner.load_env_local()


def env_override(m):
    return NekosiaNekoTestRunner.env_override(m)


def entity_data(v):
    return NekosiaNekoTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return NekosiaNekoTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return NekosiaNekoTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return NekosiaNekoTestRunner.load_test_control()


def live_client_options():
    return NekosiaNekoTestRunner.live_client_options()


def live_delay_ms():
    return NekosiaNekoTestRunner.live_delay_ms()
