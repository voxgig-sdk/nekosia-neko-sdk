...
from nekosianeko_sdk.nekosianeko_types import (
    Booru,
    BooruLoadMatch,
    BooruListMatch,
    BooruCreateData,
...
class BooruEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return BooruEntity(self._client, opts)
    ...
    def load(self, reqmatch=None, ctrl=None) -> Booru:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.Booru().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[Booru]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.Booru().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
        ctx = utility.make_context({
            "opname": "list",
    ...
    def create(self, reqdata: BooruCreateData, ctrl=None) -> Booru:
        utility = self._utility
        ctx = utility.make_context({
            "opname": "create",
            "ctrl": ctrl,
