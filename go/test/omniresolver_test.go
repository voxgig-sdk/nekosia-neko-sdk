...
import (
	...
	omni "github.com/voxgig-sdk/nekosia-neko-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.NekosiaNekoError); is {
				out := map[string]any{
					"name":    "NekosiaNekoError",
...
func omniCtx(arg any, client *sdk.NekosiaNekoSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.NekosiaNekoSDK); is {
			client = live
