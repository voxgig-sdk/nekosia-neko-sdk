...
func TestBooruEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.Booru(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.Booru(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		booruRef01Data := core.ToMapAny(vs.GetProp(
			vs.GetPath(setup.data, []any{"new", "booru"}), "booru_ref01"))

		booruRef01DataResult, err := booruRef01Ent.Create(booruRef01Data, nil)
		if err != nil {
			t.Fatalf("create failed: %v", err)
		}
		booruRef01Data = core.ToMapAny(entityData(booruRef01DataResult))
		...
		booruRef01DataDt0LoadResult := core.ToMapAny(entityData(booruRef01DataDt0Loaded))
...
func booruBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("NEKOSIA_NEKO_TEST_BOORU_ENTID")
	...
	idmapResolved := core.ToMapAny(env["NEKOSIA_NEKO_TEST_BOORU_ENTID"])
