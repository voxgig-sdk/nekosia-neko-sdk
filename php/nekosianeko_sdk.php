...
class NekosiaNekoSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = NekosiaNekoHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = NekosiaNekoHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!NekosiaNekoFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, NekosiaNekoFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return NekosiaNekoUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = NekosiaNekoHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = NekosiaNekoHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = NekosiaNekoHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new NekosiaNekoError($op . "_allow",
                "NekosiaNekoSDK: " . $op . ": operation not allowed by" .
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = NekosiaNekoHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new NekosiaNekoError("graphql_error",
                "NekosiaNekoSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    public function Booru($data = null)
    {
        ...
        if ($data === null) {
            if ($this->_booru === null) {
                $this->_booru = new BooruEntity($this, null);
            }
            return $this->_booru;
        }
        return new BooruEntity($this, $data);
    }
    ...
    public static function test(?array $testopts = null, ?array $sdkopts = null): self
    {
        ...
        $sdk = new NekosiaNekoSDK($sdkopts);
        $sdk->mode = "test";
        return $sdk;
    }
}
