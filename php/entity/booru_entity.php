...
class BooruEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new BooruEntity($this->_client, $opts);
    }

    /**
     * @param Booru|array $args Booru data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = NekosiaNekoHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return Booru|array The current Booru data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of Booru fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = NekosiaNekoHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of Booru fields).
     */
     ...
     * Load a single Booru.
     *
     * @param BooruLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed BooruLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Booru|array The loaded Booru as an assoc-array at the
     *   SDK boundary; throws NekosiaNekoError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = NekosiaNekoHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
     ...
     *
     * @param BooruListMatch|array|null $reqmatch Match filter (any subset
     *   of Booru fields) as an assoc-array; BooruListMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Booru[]|array A list of Booru items as assoc-arrays at
     *   the SDK boundary; throws NekosiaNekoError on failure (item-5 convention).
     */
     ...
     * Create a new Booru.
     *
     * @param BooruCreateData|array|null $reqdata Body data as an assoc-array;
     *   a typed BooruCreateData names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Booru|array The created Booru as an assoc-array at the
     *   SDK boundary; throws NekosiaNekoError on failure (item-5 convention).
     */
