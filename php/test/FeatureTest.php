...
class FtCtrl extends NekosiaNekoControl
{
...
class FtClient
{
    ...
    private NekosiaNekoUtility $utility;

    public function __construct(NekosiaNekoUtility $utility, array $options)
    {
    ...
    public function get_utility(): NekosiaNekoUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (NekosiaNekoContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public NekosiaNekoContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (NekosiaNekoContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = NekosiaNekoConfig::make_feature($name);
    ...
    private static function buildUrl(NekosiaNekoSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof NekosiaNekoError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new NekosiaNekoResponse($fetched);
        }
    ...
    private function fail(NekosiaNekoContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (NekosiaNekoContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
