...
class BooruEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = NekosiaNekoSDK::test(null, null);
        $ent = $testsdk->Booru(null);
    ...
    public function test_stream(): void
    {
        ...
        $base = NekosiaNekoSDK::test($seed, null);
        $seen = iterator_to_array($base->Booru(null)->stream("list", null, null), false);
        $this->assertCount(3, $seen);

        // Inbound: streaming active -> yields each item from the feature.
        $cfg = NekosiaNekoConfig::make_config();
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            $sdk = NekosiaNekoSDK::test($seed, ["feature" => ["streaming" => ["active" => true]]]);
            $got = [];
            foreach ($sdk->Booru(null)->stream("list", null, null) as $item) {
    ...
    {
        $setup = booru_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["create", "list", "load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "booru." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        // CREATE
        $booru_ref01_ent = $client->Booru(null);
...
function booru_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/booru/BooruTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["booru01", "booru02", "booru03"] as $k) {
